...
//
// 매니저는 키 생성 로직과 도메인 타입을 모른다. 키는 호출자가 문자열로 만들어 넘기고,
...
// subscriber는 하나의 키에 구독 중인 클라이언트 집합이다.
type subscriber[C comparable] struct {
	mu      sync.Mutex
	clients []C
}

func newSubscriber[C comparable]() *subscriber[C] {
	return &subscriber[C]{clients: make([]C, 0)}
}

func (s *subscriber[C]) hasUnsafe(client C) bool {
...
func (s *subscriber[C]) append(client C) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *subscriber[C]) delete(client C) int {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *subscriber[C]) snapshot() []C {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// Manager는 키별 구독자를 관리한다. 동시성 안전.
type Manager[C comparable] struct {
	mu         sync.Mutex
	records    map[string]*subscriber[C] // key → 구독자
...
// NewManager는 직렬화/전송 전략을 주입받아 매니저를 만든다.
// Subscribers만 쓰고 Publish를 쓰지 않을 거라면 marshal/send에 nil을 줘도 된다.
func NewManager[C comparable](
	marshal func(any) ([]byte, error),
	send func(C, []byte) error,
) *Manager[C] {
	return &Manager[C]{
		records:    make(map[string]*subscriber[C]),
...
func (m *Manager[C]) getOrCreateUnsafe(key string, create bool) *subscriber[C] {
...
func (m *Manager[C]) Subscribe(key string, client C) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager[C]) Unsubscribe(key string, client C) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager[C]) UnsubscribeAll(client C) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Manager[C]) unsubscribeUnsafe(key string, client C) {
...
func (m *Manager[C]) Subscribers(key string) []C {
	m.mu.Lock()
	defer m.mu.Unlock()
	sc, ok := m.records[key]
	if !ok {
...
func (m *Manager[C]) Publish(key string, payload any) error {
