...
func (r *supervisorRouter) register(conn *transport.Conn, body *protocol.RegisterRequest) transport.Handler {
	...
	return func(req protocol.Frame) (any, error) {
		...
		// [임시 테스트] 등록 직후 파일 전송 시도.
		// SendFile은 블로킹(재시도 루프)이라 반드시 고루틴으로 — 안 그러면
		// 이 register 응답이 전송 끝날 때까지 안 나가 worker register가 타임아웃.
		// reader, err := transfer.NewReadFile("/home/jh-bae/다운로드/SHA256SUMS", 0)
		// if err != nil {
		// 	log.Printf("[SUPER_TEST_AFTER_REGISTER] %v", err)
		// } else {
		// 	go r.SendFile(key, reader)
		// }
